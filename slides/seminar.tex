...
\begin{frame}{Three moments}
  ...
  \begin{tikzpicture}[
    ...
    \node[event, fill=tjo@lightgrey] (gpt) at (-4.2,0) {%
    ...
    \node[event, fill=tjo@lightgrey] (good) at (4.2,0) {%
...
\begin{frame}{Quick survey: experience}
  ...
  {\fontsize{20}{26}\selectfont
    \begin{tabular}{l}
      \color{tjo@cyan}$\bullet$\quad ChatGPT, Claude, or Gemini? \\[0.8em]
      \color{tjo@cyan}$\bullet$\quad An LLM in your IDE to write code? \\[0.8em]
      \color{tjo@cyan}$\bullet$\quad Coding agents? \\
    \end{tabular}
  }
  \vfill
\end{frame}
...
% --- Personal confession ---
\statementslide{If you tried chat in Q1 2025:\\[0.5em]
  you probably dismissed LLMs for research\\[0.8em]
...
\begin{frame}{The evidence}
  \vfill
  \begin{flushleft}
  \hspace{1.5em}Seductively correct equations that are nonsense

  \vskip1.5em
  \hspace{1.5em}Push back on a valid proof and the LLM finds a ``bug'' that isn't there

  \vskip1.5em
  \hspace{1.5em}They lie, cheat, steal, and flatter
  \end{flushleft}
...
% TODO: Insert slide here — screenshot of auto formalization project
...
\statementslide{An LLM outputs a probability distribution over tokens\\[0.8em]
  Then it samples one after another}
...
\begin{frame}{Under the hood: Transformers}
  \vfill
  \begin{flushleft}
  \hspace{1.5em}The neural network inside is a \textbf{Transformer}

  \vskip1.5em
  \hspace{1.5em}{\small\color{tjo@black!60}
    \begin{tabular}{@{}l@{\hspace{1em}}l}
      Vaswani et al. & \textit{Attention Is All You Need} (2017) \\[0.4em]
      Andrej Karpathy & nanoGPT --- build a GPT from scratch \\[0.4em]
      Jay Alammar & \textit{The Illustrated Transformer} \\[0.4em]
      PicoGPT.jl & minimal GPT in Julia (my repo) \\
    \end{tabular}
  }
  \end{flushleft}
  \vfill
...
\begin{frame}{Two ways in}
  ...
  \begin{tikzpicture}[node distance=4cm]
    ...
    \draw[tjo arrow, <->] ([yshift=-0.3cm]web.east) -- ([yshift=-0.3cm]term.west)
...
% --- Coding agents ---
\statementslide{Coding agents\\[0.8em]
  {\fontsize{20}{26}\selectfont\mdseries
    Empowering LLMs with autonomous tool use\\[0.3em]
...
\begin{frame}{Tools}
  ...
  \vskip2em
  {\small\color{tjo@black!60}
    Thorsten Ball, \textit{How to Build an Agent} --- ampcode.com}
  \vfill
\end{frame}
...
\begin{frame}{Automation}
  ...
  \begin{tikzpicture}
    % The spectrum line
    \draw[tjo@darkteal, line width=2pt] (-5,0) -- (5,0);
    % Gradient zone in the middle
    \shade[left color=tjo@lightgrey, right color=tjo@cyan!30]
      (-0.8,-0.15) rectangle (0.8,0.15);
    % Labels
    \node[above, font=\bfseries\large, text=tjo@darkteal] at (-5,0.3) {Labour};
    \node[above, font=\bfseries\large, text=tjo@darkteal] at (5,0.3) {Cognition};
    % Points on the spectrum
    \fill[tjo@darkteal] (-4,0) circle (4pt);
    \node[below=8pt, font=\normalsize] at (-4,0) {Calculator};
    \fill[tjo@darkteal] (-1.5,0) circle (4pt);
    \node[below=8pt, font=\normalsize] at (-1.5,0) {Computer algebra};
    \fill[tjo@darkteal] (1.5,0) circle (4pt);
    \node[below=8pt, font=\normalsize] at (1.5,0) {Automated proving};
    \fill[tjo@cyan] (4,0) circle (4pt);
    \node[below=8pt, font=\normalsize, text=tjo@darkteal] at (4,0) {\textbf{AI tools}};
  \end{tikzpicture}
  \vfill
\end{frame}
