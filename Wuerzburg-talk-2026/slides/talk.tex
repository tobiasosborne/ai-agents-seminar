...
\begin{frame}{Closing the loop}
  ...
  \begin{tikzpicture}[every node/.style={align=center}]
    \node[tjo box, minimum width=4.4cm, minimum height=1.5cm, font=\small] (chat)
      at (-4,0) {\textbf{Chat}\\[3pt]{\small closed loop, through \emph{you}}\\[2pt]
        {\color{red!70!black}you cut, paste, re-prompt}};
    \node[tjo box fill, minimum width=4.4cm, minimum height=1.5cm, font=\small] (agent)
      at (4,0) {\textbf{Agent}\\[3pt]{\small closed loop, through tools}\\[2pt]
        {\color{tjo@darkteal}results feed back automatically}};

    \draw[tjo arrow, line width=2pt] (chat.east) -- (agent.west)
      node[midway, above=8pt, font=\footnotesize, text=tjo@black!60] {same loop, no human relay};
  \end{tikzpicture}
  ...
  \centering
  {\color{tjo@darkteal}\bfseries
    The agent closes the same loop faster, with no human cutting and pasting}
  \vfill
\end{frame}
...
\begin{frame}{Vibefeld: molecular pedantry}
  ...
  \centering
  \begin{tikzpicture}[every node/.style={align=center}]
    \node[tjo box, minimum width=3.4cm, minimum height=1.5cm, font=\small] (claim)
      at (-4.6,0) {\textbf{Claim 1}\\{\small the theorem}};
    \node[tjo box fill, minimum width=3.4cm, minimum height=1.5cm, font=\small] (steps)
      at (0,0) {\textbf{1.1, 1.2, 1.1.1, \ldots}\\{\small atomic steps}};
    \node[tjo box, fill=tjo@darkteal, text=white, minimum width=3.4cm, minimum height=1.5cm, font=\small] (val)
      at (4.6,0) {\textbf{Validated}\\{\small survived attack}};
    \draw[tjo arrow] (claim) -- (steps)
      node[midway, above=6pt, font=\footnotesize, text=tjo@black!60] {decompose};
    \draw[tjo arrow] (steps) -- (val)
      node[midway, above=6pt, font=\footnotesize, text=tjo@black!60] {challenge};
  \end{tikzpicture}
...
\begin{frame}{Vibefeld: how trust accumulates}
  \vfill
  \begin{flushleft}
  \hspace{1.5em}\textbf{Provers propose}, \textbf{verifiers attack}: a claim is accepted only when every challenge is resolved

  \vskip0.8em
  \hspace{1.5em}Every proposal, challenge, and resolution lands in an append-only ledger: a full audit trail

  \vskip0.8em
  \hspace{1.5em}Assumptions stay visible: an \emph{admitted} claim taints everything that depends on it
  \end{flushleft}
